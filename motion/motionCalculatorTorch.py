...
from motion.motionMatrixTorch import RotateType
import math
...
class MotionCalculator:
    ...
    def calculate(self, x, y, z, c, a):
        ...
        self.__pIdeal = MotionMatrixTensor(x, y, z).rotateZ(c) * self.__BMat * MotionMatrixTensor().rotateX(a)
        # self.__pIdeal = MotionMatrixTensor(x, y, z).rotateZ(c) * MotionMatrixTensor().rotateX(a)
        self.__pIdeal = self.__pIdeal * self.__LMat
        # x静态误差
        # 平移x带随机误差，直线度随机y,z,随机旋转δα、δβ、δγ
        e_static_x = MotionMatrixTensor(self.__randomError(x, _x), _randomTrans(self.__straightness[_x][0]),
        ...
        e_static_y = MotionMatrixTensor(_randomTrans(self.__straightness[_y][0]), self.__randomError(y, _y),
        ...
        e_static_z = MotionMatrixTensor(_randomTrans(self.__straightness[_z][0]),
                                        _randomTrans(self.__straightness[_z][1]),
        ...
        e_static_sp = MotionMatrixTensor(_randomTrans(), _randomTrans(SP_PRECISION), 0.0, _randomRotate(),
                                         _randomRotate(),
        ...
        e_v_xy = MotionMatrixTensor().rotateZ(rd(self.__verticalPre[0]), rtype)
        e_v_zx = MotionMatrixTensor().rotateY(rd(self.__verticalPre[1]), rtype)
        e_v_zy = MotionMatrixTensor().rotateX(rd(self.__verticalPre[2]), rtype)
        e_v_cx = MotionMatrixTensor().rotateY(rd(self.__verticalPre[3]), rtype)
        e_v_cy = MotionMatrixTensor().rotateX(rd(self.__verticalPre[4]), rtype)
        e_v_cb = MotionMatrixTensor().rotateX(rd(self.__verticalPre[5]), rtype)
        e_v_ca = MotionMatrixTensor().rotateY(rd(self.__verticalPre[6]), rtype)

        # e_v_xy = MotionMatrixTensor().rotateZ(rd(vp), rtype)
        # e_v_zx = MotionMatrixTensor().rotateY(rd(vp), rtype)
        # e_v_zy = MotionMatrixTensor().rotateX(rd(vp), rtype)
        # e_v_cx = MotionMatrixTensor().rotateY(rd(vp), rtype)
        # e_v_cy = MotionMatrixTensor().rotateX(rd(vp), rtype)
        # e_v_cb = MotionMatrixTensor().rotateX(rd(VERTICAL_PRECISION), rtype)
        # e_v_ca = MotionMatrixTensor().rotateY(rd(vp), rtype)

        # 实际精度
        self.__pActual = MotionMatrixTensor().transX(x) * e_static_x * e_v_xy * \
                         MotionMatrixTensor().transY(y) * e_static_y * e_v_zx * e_v_zy * \
                         MotionMatrixTensor().transZ(z) * e_static_z * e_v_cx * e_v_cy * \
                         MotionMatrixTensor().rotateZ(c) * e_static_c * self.__BMat * e_v_cb * e_v_ca * \
                         MotionMatrixTensor().rotateX(a) * e_static_a * MotionMatrixTensor() * e_static_sp

        # self.__pActual = MotionMatrixTensor().transX(x) * e_static_x * e_v_xy * \
        #                  MotionMatrixTensor().transY(y) * e_static_y * e_v_zx * e_v_zy * \
        #                  MotionMatrixTensor().transZ(z) * e_static_z * e_v_cx * e_v_cy * \
        #                  MotionMatrixTensor().rotateZ(c) * e_static_c * e_v_cb * e_v_ca * \
        #                  MotionMatrixTensor().rotateX(a) * e_static_a * MotionMatrixTensor() * e_static_sp
        self.__pActual = self.__pActual * self.__LMat
        # print((MotionMatrixTensor().rotateX(c)).matrix)
        # 误差
        self.__pError = self.__pActual.matrix - self.__pIdeal.matrix
        # p_error_list = self.__pError.tolist()
        m1 = self.__pError[0][0]
        m2 = self.__pError[1][0]
        m3 = self.__pError[2][0]

        e_dis = torch.sqrt(m1 * m1 + m2 * m2 + m3 * m3)
        # print("loc_z")
    ...
    def setBiasACY(self, bias):
        self.__B_ACY = bias
        self.__BMat = MotionMatrixTensor(0.0, bias)

    # 摆长
    def setL(self, length):
        self.__L = length
        self.__LMat = torch.tensor([[0], [0], [-self.__L], [1]], dtype=torch.double, device=MotionMatrixTensor.device)
